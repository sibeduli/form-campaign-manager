...
class FormLinkAction:
    ...
    def get_all(self):
        try:
            ...
            for form_link in form_links:
                ...
                form_link.url_link = f'http://{request.host}/guest-form/{form_link.uuid}'
    ...
    def add(self):
        try:
            ...
            db.session.commit()
            return jsonify({'message': 'Form link added successfully', 'data': new_form_link.to_dict()}), 200
        except Exception as e:
            db.session.rollback()
    ...
    def get_last_form_link(self):
        try:
            ...
            print(f'Error in get_last_form_link action: {str(e)}\n{traceback.format_exc()}')
            return None
    ...
    def update_viewed_time(self, id):
        try:
            ...
            if form_link:  # Check if form_link exists
                form_link.viewed_time = datetime.now(timezone.utc) + timedelta(hours=7)  # Convert to GMT+7
                db.session.commit()
                return True
    ...
    def update_submitted_time(self, id):
        try:
            form_link = FormLink.query.get(id)
            form_link.submitted_time = datetime.now(timezone.utc) + timedelta(hours=7)  # Convert to GMT+7
            db.session.commit()
            return True
        except Exception as e:
    ...
    def get_last_form_link_submitted(self):
        try:
            form_link = FormLink.query.where(FormLink.submitted_time.isnot(None)).order_by(FormLink.submitted_time.desc()).first()
            if not form_link:
                return None
            return form_link.to_dict()
        except Exception as e:
            print(f'Error in get_last_form_link_submitted action: {str(e)}\n{traceback.format_exc()}')
            return None
        
    def get_last_form_link_viewed(self):
        try:
            form_link = FormLink.query.where(FormLink.viewed_time.isnot(None)).order_by(FormLink.viewed_time.desc()).first()
            if not form_link:
                return None
            return form_link.to_dict()
        except Exception as e:
            print(f'Error in get_last_form_link_viewed action: {str(e)}\n{traceback.format_exc()}')
            return None
    ...
    def get_unopened_form_count(self):
        try:
            unopened_form_count = FormLink.query.filter(FormLink.status == 'unopened').count()
            return unopened_form_count
        except Exception as e:
            print(f'Error in get_unopened_form_count action: {str(e)}\n{traceback.format_exc()}')
            return 0
    ...
    def get_last_form_link_created(self):
        try:
            ...
            print(f'Error in get_last_form_link_created action: {str(e)}\n{traceback.format_exc()}')
            return None
