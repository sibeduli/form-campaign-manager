...
class CampaignAction:
    ...
    def _generate_responden_id(self, name):
        ...
        initials = (
            "".join(word[0].upper() for word in name_parts) if name_parts else "XX"
        )
    ...
    def edit(self, id, data):
        try:
            ...
            if (
                "responden_name" in data
                and data["responden_name"] != campaign.responden_name
            ):
    ...
    def delete(self, id):
        try:
            ...
            db.session.rollback()
            current_app.logger.error(
                f"Error in delete action: {str(e)}\n{traceback.format_exc()}"
            )
            return jsonify({"error": str(e)}), 500
    ...
    def pdf(self, id):
        try:
            ...
            response = send_file(
                ...
                headers={"X-Filename": filename},  # Simple, direct header
            )
