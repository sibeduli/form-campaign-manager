...
def pdf_campaign(id):
    try:
        ...
        safe_campaign_name = "".join(
            c for c in campaign.campaign_name if c.isalnum() or c in (" ", "-", "_")
        ).strip()
        filename = (
            f"campaign-{creation_date}-{campaign.responden_id}-{safe_campaign_name}.pdf"
        )
...
def copy_form_link(id):
    try:
        ...
        return (
            jsonify({"message": "Form link copied successfully", "url_link": url_link}),
            200,
        )
    except Exception as e:
        log_history_action.add(
...
@app.route(
    "/api/form-link-manager/action/get-last-form-link-submitted", methods=["GET"]
)
def get_last_form_link_submitted():
    try:
        ...
        print(
            f"Error in get_last_form_link_submitted: {str(e)}\n{traceback.format_exc()}"
        )
        return jsonify({"error": str(e)}), 500
        ...
        while True:
            ...
            with app.app_context():
                ...
                data = {
                    ...
                    "last_form_id_created": (
                        last_form_created["id"] if last_form_created else "-"
                    ),
                    "last_form_id_submitted": (
                        last_form_submitted["id"] if last_form_submitted else "-"
                    ),
                    "last_form_id_viewed": (
                        last_form_viewed["id"] if last_form_viewed else "-"
                    ),
                }
